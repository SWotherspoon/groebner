...
Alternately, if a common right eigenbasis is calculated from the transpose of the multiplication 
matrices
```{r}
rMs <- lapply(Ms,t)
rBs <- common_eigenbasis(rMs)
```
then roots can be found directly from the (right) eigenbasis provided that each standard variable
appears as an element of the monomial basis
```{r}
roots_eigenvectors(ms,rBs)
```
